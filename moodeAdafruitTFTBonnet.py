    ...
    if act_mpd == True:
        while True:
            ...
            if button_B.value == False and button_A.value == True:  # B Button - Load default Playlist.                
                 draw.rectangle((0,0,WIDTH,HEIGHT),fill=DISP_BACKGROUND_COLOR)
                 draw.rectangle((0, 135, WIDTH, 210), (255,255,255,145))
                 w3, y3 = draw.textsize('Lade Playlist', font_l)
                 x3 = x3-20
                 if x3 < (WIDTH - w3 - 20):
                     x3 = 0
                 if w3 <= WIDTH:
                     x3 = (WIDTH - w3)//2
                 if SHADE != 0:
                     draw.text((x3, 140+SHADE), 'Lade Playlist', font=font_l, fill=FONT_SHADOW_COLOR)
                 draw.text((x3, 140), 'Lade Playlist', font=font_l, fill=FONT_COLOR)
                
                 w3, y3 = draw.textsize(DEFAULT_PLAYLIST, font_l)
                 x3 = x3-20
                 if x3 < (WIDTH - w3 - 20):
                     x3 = 0
                 if w3 <= WIDTH:
                     x3 = (WIDTH - w3)//2
                 if SHADE != 0: 
                     draw.text((x3, 170+SHADE), DEFAULT_PLAYLIST, font=font_l, fill=FONT_SHADOW_COLOR)
                 draw.text((x3, 170), DEFAULT_PLAYLIST, font=font_l, fill=FONT_COLOR)
                 os.system('mpc clear')
                 os.system('mpc load \'' + DEFAULT_PLAYLIST + '\'')
                 os.system('mpc play')
